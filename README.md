...
- Example : Diner has to authenticate its staff and A Diner staff can only access kitchen. The guard cannot not allow unauthenticated staff. To avoid redundant authentication, staff is provided a badge (JWT) which allows them to access kitchen instead of going to authentication process everyday.
